class_name ReaperShop
extends Control

signal upgrade_bought(key: String)
signal closed()

var _data: ReaperShopData = null
var _gold: int = 0
...
func open(data: ReaperShopData, gold: int) -> void:
	_data = data
	_gold = gold
	_refresh()
	visible = true
...
func _build_ui() -> void:
	...
	ps.bg_color = Color(0.10, 0.05, 0.15, 0.98)
	ps.set_corner_radius_all(10)
	ps.set_border_width_all(2)
	ps.border_color = Color(0.55, 0.20, 0.80, 1.0)
...
func _make_inner_panel() -> Panel:
	...
	s.bg_color = Color(0.08, 0.04, 0.12, 1.0)
	s.set_corner_radius_all(6)
	s.set_border_width_all(1)
	s.border_color = Color(0.40, 0.15, 0.60, 0.7)
...
func _refresh_info() -> void:
	...
	_info_vbox.add_child(_make_section_label("Doom", Color(0.85, 0.55, 1.0)))
	_info_vbox.add_child(HSeparator.new())
	_info_vbox.add_child(_make_stat_row("Doom Damage", "%d per trigger" % _data.doom_damage))
	var aura_txt := "+%d/+%d per doom counter" % [_data.reaper_aura_bonus, _data.reaper_aura_bonus] if _data.reaper_aura_bonus > 0 else "Not purchased"
	_info_vbox.add_child(_make_stat_row("Reaper Aura", aura_txt))
	_info_vbox.add_child(HSeparator.new())

	var desc := Label.new()
	desc.text = "Doom is applied by unit abilities. Every time any friendly unit attacks, all doomed enemies lose 1 counter. Applying doom to an already-doomed enemy reduces its counters by 4. At 0 counters, the unit takes Doom Damage. Reapers gain +N/+N per total doom counter on the field."
	desc.autowrap_mode = TextServer.AUTOWRAP_WORD
	desc.add_theme_font_size_override("font_size", 11)
	desc.add_theme_color_override("font_color", Color(0.55, 0.40, 0.65))
...
func _refresh_upgrades() -> void:
	...
	_upgrades_vbox.add_child(_make_section_label("Upgrades", Color(0.85, 0.55, 1.0)))
	_upgrades_vbox.add_child(HSeparator.new())

	_upgrades_vbox.add_child(_make_upgrade_row(
		"Doom Damage +25",
		"Doom currently deals %d damage on expiry." % _data.doom_damage,
		_data.damage_cost(),
		"doom_damage"
	))
	_upgrades_vbox.add_child(HSeparator.new())

	var aura_label := "Reaper Aura +1/+1"
	var aura_desc: String
	if _data.reaper_aura_bonus == 0:
		aura_desc = "First purchase unlocks aura: Reapers gain +1/+1 per doom counter on the field."
	else:
		aura_desc = "Reapers currently gain +%d/+%d per doom counter (real-time)." % [_data.reaper_aura_bonus, _data.reaper_aura_bonus]
	_upgrades_vbox.add_child(_make_upgrade_row(
		aura_label,
		aura_desc,
		_data.aura_cost(),
...
func _make_upgrade_row(label_text: String, desc_text: String, cost: int, key: String) -> HBoxContainer:
	...
	desc_lbl.add_theme_color_override("font_color", Color(0.50, 0.32, 0.65))
...
func _make_stat_row(left_text: String, right_text: String, right_color: Color = Color(0.75, 0.50, 0.95)) -> HBoxContainer:
	var row := HBoxContainer.new()
	var lbl := Label.new()
	lbl.text = left_text
